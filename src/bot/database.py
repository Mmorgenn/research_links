...
class VectorDB:
    ...
    async def add_user(self, form: dict[str, str], username: str, chat_id: str) -> None:
        ...
        await user_collection.add(
            ids=[self.user_id], documents=[str(form.get("info", "Errors"))], metadatas=[form], embeddings=[embedding]
        )
    ...
    async def update_form(self, form_change: str, new_value: str) -> None:
        ...
        if form_change == "info":
            ...
            await user_collection.update(
                ids=[self.user_id], documents=[new_value], metadatas=[new_metadatas], embeddings=[embedding]
            )
        else:
    ...
    async def get_similar_form(self) -> tuple[dict[str, str], str] | None:
        user_collection = await self.client.get_or_create_collection("research_links")
        user_form = await user_collection.get(
            ids=[self.user_id], include=[IncludeEnum.metadatas, IncludeEnum.embeddings]
        )

        if not (
            isinstance(user_form["metadatas"], list)
            and isinstance(user_form["metadatas"][0], dict)
            and isinstance(user_form["embeddings"], np.ndarray)
        ):
            return None
        ...
        if (
            ...
            or not isinstance(other_form["metadatas"][0], list)
            or not bool(other_form["metadatas"][0])
            or not isinstance(other_form["metadatas"][0][0], dict)
        ):
            return None
    ...
    async def match(self, other_id: str) -> None:
        ...
        if (
            ...
            and isinstance(metadatas[1], dict)
        ):
