...
from src.bot.routers.form_router import POSSIBLE_STATUS, POSSIBLE_AREA
...
async def request_change(call: CallbackQuery, state: FSMContext) -> None:
    ...
    form = str(call.data)
    await delete_message(call.message)
    await call.message.answer(text=response.get(f"new_{form}", "Ошибка! /menu"),
                              reply_markup=keyboards.get(form))
...
async def confirm_change(message: Message, state: FSMContext) -> None:
    ...
    bot_message = await message.answer(text=response["change"],
                                       reply_markup=MenuK.back_keyboard("setting"))
...
setting_router.message.register(confirm_change, SettingState.status,
                                lambda message: message.text.title() in POSSIBLE_STATUS)
